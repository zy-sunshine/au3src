...
char ParserExp::m_PrecOpRules[OPR_MAXOPR][OPR_MAXOPR] = {
/* Stack    ------------------ Input -------------------- */
/*        <   >   <=  >=  <>  =   == AND  OR  &  NOT  +   -   *   /   ^   M   P,  (   )   END */
...
void ParserExp::ExpandEnvString(Variant &vString)
{
    AString    sResult;
    AString sName;
...
void ParserExp::ExpandVarString(Variant &vString)
{
    AString    sResult;
    AString sName;
    int        nLine = 0;                    // In and out string positions
    ...
    while ( (ch = szLine[nLine++]) != '\0')
    {
        ...
        else if ( ch == '@' )
        {
            ...
            else
            {
                ...
                // Get the value of this variable and copy it to our result string
                if ( AUT_SUCCEEDED(EvaluateMacro(sName.c_str(), vTemp)) )
...
AUT_RESULT ParserExp::EvaluateVariable(VectorToken &vLineToks, unsigned int &ivPos, Variant &vResult)
{
    Variant        *pvTemp;
    bool        bConst = false;
    Variant        vTemp;
...
AUT_RESULT ParserExp::GetArrayElement(VectorToken &vLineToks, uint &ivPos, Variant **ppvTemp)
{
    Variant vTemp;
    int        nColVar = -1;
    int        nColTemp = -1;
...
AUT_RESULT ParserExp::EvaluateMacro(const char *szName, Variant &vResult)
{
...
AUT_RESULT ParserExp::EvaluateCondition(VectorToken &vLineToks, unsigned int &ivPos, bool &bResult)
{
    Variant    vTemp;
...
AUT_RESULT ParserExp::EvaluateExpression(VectorToken &vLineToks, unsigned int &ivPos, Variant &vResult)
{
    ...
    for (;;)
    {
        ...
        switch ( vLineToks[ivPos].m_nType )
        {
            ...
            case TOK_VARIABLE:
                if ( AUT_FAILED(EvaluateVariable(vLineToks, ivPos, vTemp)) )
                    return AUT_ERR;
...
AUT_RESULT ParserExp::OprReduce(StackInt &opStack, StackVariant &valStack)
{
    Variant    vOp1, vOp2;
...
AUT_RESULT ParserExp::SkipBoolean(VectorToken &vLineToks, unsigned int &ivPos)
{
    int nCount = 0;
    int    tok;
